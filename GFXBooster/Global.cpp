...
// -----------------------------------------------------------------------------
// CommonLibF4 compatibility shim
// -----------------------------------------------------------------------------
namespace RE
{
	BSGraphics::RendererData* GetRendererData() noexcept
	{
		// Some CommonLibF4 builds don't provide an official way to fetch renderer data.
		// For build-compatibility, we provide a dummy instance. Callers must null-check
		// device/context/swapChain before using.
		static RendererData s_dummy{};
		return &s_dummy;
	}
}
